...
//    5. Onboarding Complete — emails candidate when HR marks as Onboarded
// ============================================================
// ── CONFIG ──────────────────────────────────────────────────
...
//  Called by backend when HR clicks "Mark Onboarded"
//  Payload: { action, email, fullName, position, joiningDate }
// ════════════════════════════════════════════════════════════
...
function _sendApplicationConfirmation(c) {
  ...
  const body =
    '<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;border:1px solid ' + C_GRAY_BORDER + ';border-radius:12px;overflow:hidden;">' +
      ...
      _emailFooter() +
    '</div>';
  GmailApp.sendEmail(c.email, subject, "", { htmlBody: body, from: HR_EMAIL, replyTo: HR_EMAIL, name: COMPANY_NAME + " - Talent Acquisition" });
}
...
function _sendOfferLetterEmail(c, attachment) {
  ...
  const body =
    '<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;border:1px solid ' + C_GRAY_BORDER + ';border-radius:12px;overflow:hidden;">' +
      ...
      _emailFooter() +
    '</div>';
  const options = { htmlBody: body, from: HR_EMAIL, replyTo: HR_EMAIL, name: COMPANY_NAME + " - Talent Acquisition" };
...
// ════════════════════════════════════════════════════════════
//  EMAIL 9 — Onboarding Complete (docs verified, ready to join)
// ════════════════════════════════════════════════════════════
function _sendOnboardedEmail(c) {
      ...
      '<div style="padding:32px;background:' + C_WHITE + ';">' +
        ...
        '<div style="background:#F9F9F9;border:1px solid ' + C_GRAY_BORDER + ';border-radius:8px;padding:20px;margin-bottom:24px;">' +
          ...
          '<div style="display:flex;align-items:center;margin-bottom:10px;">' +
            '<div style="min-width:24px;height:24px;background:' + C_LIME + ';color:' + C_BLACK + ';border-radius:50%;text-align:center;line-height:24px;font-size:11px;font-weight:bold;margin-right:12px;flex-shrink:0;">V</div>' +
            '<span style="font-size:13px;color:' + C_GRAY_DARK + ';">Offer Letter Accepted</span>' +
          '</div>' +
          '<div style="display:flex;align-items:center;margin-bottom:10px;">' +
            '<div style="min-width:24px;height:24px;background:' + C_LIME + ';color:' + C_BLACK + ';border-radius:50%;text-align:center;line-height:24px;font-size:11px;font-weight:bold;margin-right:12px;flex-shrink:0;">V</div>' +
            '<span style="font-size:13px;color:' + C_GRAY_DARK + ';">Documents Submitted</span>' +
          '</div>' +
          '<div style="display:flex;align-items:center;margin-bottom:10px;">' +
            '<div style="min-width:24px;height:24px;background:' + C_LIME + ';color:' + C_BLACK + ';border-radius:50%;text-align:center;line-height:24px;font-size:11px;font-weight:bold;margin-right:12px;flex-shrink:0;">V</div>' +
            '<span style="font-size:13px;color:' + C_GRAY_DARK + ';">Documents Verified by HR</span>' +
          '</div>' +
          '<div style="display:flex;align-items:center;">' +
            '<div style="min-width:24px;height:24px;background:' + C_BLACK + ';color:' + C_LIME + ';border-radius:50%;text-align:center;line-height:24px;font-size:13px;font-weight:bold;margin-right:12px;flex-shrink:0;">★</div>' +
            '<span style="font-size:13px;font-weight:bold;color:' + C_GRAY_DARK + ';">Ready to Join ' + COMPANY_NAME + '!</span>' +
          '</div>' +
        '</div>' +
  ...
  GmailApp.sendEmail(c.email, subject, "", {
    ...
    from    : HR_EMAIL,
    replyTo : HR_EMAIL,
    name    : COMPANY_NAME + " - Onboarding Team",
  });
...
//  4. Script Properties: BACKEND_URL, ONBOARDING_WEBHOOK_SECRET, ONBOARDING_FORM_URL
//  5. Triggers > onShortlistEdit > From spreadsheet > On edit
//  6. Triggers > onFormSubmit > From form > On form submit
// ════════════════════════════════════════════════════════════
