// ============================================================
//  Google Apps Script — Interview Shortlist Email Trigger
//  Company : Zyntrix Software Solution
//  Purpose : Sends a shortlist email to the candidate when
//            "Interview Shortlisted" (Col O) becomes TRUE — either
//            via a manual edit in the Sheet (onShortlistEdit) OR via
//            the CRM "Shortlist" button (doPost web app).
// ============================================================
    ...
    '<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;border:1px solid #E5E7EB;border-radius:12px;overflow:hidden;">' +
      ...
      '<div style="background:#F0FDF4;border-bottom:1px solid #BBF7D0;padding:14px 32px;text-align:center;">' +
        '<span style="background:#059669;color:#fff;padding:6px 22px;border-radius:20px;font-size:14px;font-weight:bold;">' +
          '&#9733;&nbsp; Resume Shortlisted — Interview Scheduled Soon!' +
        '</span>' +
      '</div>' +
      ...
      '<div style="padding:32px;background:#fff;">' +
        ...
        '<p style="color:#374151;font-size:14px;line-height:1.7;margin:0 0 24px;">' +
          ...
          'your resume has been <strong style="color:#059669;">shortlisted by our recruiter</strong>. ' +
        ...
        '<div style="background:#ECFDF5;border-left:4px solid #059669;border-radius:8px;padding:18px 20px;margin-bottom:24px;">' +
          ...
          '<p style="margin:0;font-size:14px;color:#065F46;line-height:1.7;">' +
            ...
            '<strong>interview call</strong>. Please ensure your phone is reachable and keep an eye on your inbox.' +
          '</p>' +
        '</div>' +
...
// ════════════════════════════════════════════════════════════
//  TRIGGER FUNCTION — fires on every spreadsheet edit (manual UI edits)
// ════════════════════════════════════════════════════════════
function onShortlistEdit(e) {
  try {
    ...
    // ── ONLY react to Column O (1-based = 15) "Interview Shortlisted" ──
    if (editedCol !== 15) return;
    if (editedRow < 2)    return; // skip header row

    const newValue = String(e.value || "").trim().toLowerCase();
    const isShortlisted = (newValue === "true" || newValue === "yes");
    ...
    console.log("✅ Shortlist email sent → " + candidate.email);
...
// ════════════════════════════════════════════════════════════
//  WEB APP ENDPOINT — called by the Zyntrix CRM "Shortlist" button
// ════════════════════════════════════════════════════════════
//  DEPLOY: Deploy ▸ New deployment ▸ Web app
//          Execute as: Me     Who has access: Anyone
//          Copy the /exec URL → backend env var GAS_WEBAPP_URL
//          (optional) backend GAS_WEBAPP_SECRET = GAS_SHARED_SECRET above
// ════════════════════════════════════════════════════════════
function doPost(e) {
  try {
...
// ════════════════════════════════════════════════════════════
//  TRIGGER SETUP REMINDER (for the manual-edit path)
// ════════════════════════════════════════════════════════════
